...
void Framebuffer::updateVertexData(Vector2f p, Vector2f a) {
  ...
  vertices = {v4D(pos.x, pos.y, 0.0, 0.0), v4D(pos.x, pos.y + area.y, 0.0, 1.0),
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(v4D) * vertices.size(),
                  vertices.data());
...
void Framebuffer::loadQuad(Vector2f p, Vector2f a) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, VBO);
  glBufferData(GL_ARRAY_BUFFER, sizeof(v4D) * vertices.size(), vertices.data(),
               GL_DYNAMIC_DRAW);
