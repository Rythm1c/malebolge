...
void GUI::processevents(SDL_Event &event) { ImGui_ImplSDL2_ProcessEvent(&event); }
...
void GUI::load()
{
    ...
    ImGui_ImplSDL2_InitForOpenGL(Engine::getInstance()->P_window->win, Engine::getInstance()->P_window->context);
...
void GUI::editObject()
{
    ...
    ImGui::Text("objects:");
    if (ImGui::BeginListBox("##objects", ImVec2(-FLT_MIN, 4 * ImGui::GetTextLineHeightWithSpacing())))
    {
        for (int n = 0; n < this->P_world->assets->objectListSize; n++)
        {
            const bool is_selected = (objIndex == n);
            if (ImGui::Selectable(this->P_world->assets->objectList[n], is_selected))
            {
                objIndex = n;
    ...
    if (this->choosenObj != "")
    {
        ...
        this->pos = this->P_world->assets->getShape(choosenObj)->transform.translation;

       // this->rotation = this->P_world->assets->getShape(choosenObj)->transform->rotation;
    ...
    if (choosenObj != "")
    {
        if (P_world->assets->getShape(choosenObj)->transform.scaling != this->size)
        {
            P_world->assets->getShape(choosenObj)->transform.scaling = this->size;
        }

        if (P_world->assets->getShape(choosenObj)->transform.translation != this->pos)
        {
    ...
    /*     if (P_world->assets->getShape(choosenObj)->properties->rotation.x != this->rotation.x)
        {
            P_world->assets->getShape(choosenObj)->properties->rotation.x = this->rotation.x;
        }
        if (P_world->assets->getShape(choosenObj)->properties->rotation.y != this->rotation.y)
        {
            P_world->assets->getShape(choosenObj)->properties->rotation.y = this->rotation.y;
        }
        if (P_world->assets->getShape(choosenObj)->properties->rotation.z != this->rotation.z)
        {
            P_world->assets->getShape(choosenObj)->properties->rotation.z = this->rotation.z;
        } */
