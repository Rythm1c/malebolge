...
Pose &Pose::operator=(const Pose &p)
{
    ...
    {
        memcpy(&this->parents[0], &p.parents[0], sizeof(int) * this->parents.size());
    }

    if (this->joints.size() != 0)
    {
        memcpy(&this->joints[0], &p.joints[0], sizeof(Transform) * this->joints.size());
    }
...
{
    return (uint)this->joints.size();
}

Transform Pose::getLocalTransform(size_t index)
{
...
int Pose::getParent(size_t index)
{
