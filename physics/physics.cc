...
/* void Physics::gravity(Shape *Pr_mass1, Shape *Pr_mass2)
{
    ...
    float distance = get_length(Pr_mass1->transform.translation -
Pr_mass2->transform.translation );
    // direction from mass1 to mass2
    // |AB| = |AO| + |OB| = |OB| - |AO|
    v3D Obj1_Obj2 = normalize(Pr_mass2->transform.translation  -
Pr_mass1->transform.translation );
    // attenuation
...
void Physics::cAABBVsAABB(Shape *pr_obj1, Shape *pr_obj2) {
  const AABB obj1 =
      getAABB((*pr_obj1).transform.translation, (*pr_obj1).transform.scaling);
  const AABB obj2 =
      getAABB((*pr_obj2).transform.translation, (*pr_obj2).transform.scaling);
...
void Physics::cSphereVsAABB(Shape *pr_sphere, Shape *pr_aabb) {
  ...
  if (distance <= radius) {
    ...
    pr_sphere->transform.translation +=
        normal * averageSize(pr_sphere->transform) - diff;
