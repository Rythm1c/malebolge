...
void World::load()
{
    ...
    player->transform->orientation = Quat(180.0, {0.0, 1.0, 0.0});
    player->init();
...
void World::update()
{
    if (!pause)
    {
        physics->cSphereVsAABB(assets->getShape("ball1"), assets->getShape("platform"));
        physics->simpleGravity(assets->getShape("ball1")->velocity);

        physics->cSphereVsAABB(assets->getShape("ball2"), assets->getShape("platform"));
        physics->simpleGravity(assets->getShape("ball2")->velocity);

        physics->cAABBVsAABB(assets->getShape("cube1"), assets->getShape("platform"));
        physics->simpleGravity(assets->getShape("cube1")->velocity);

        physics->cAABBVsAABB(assets->getShape("cube2"), assets->getShape("platform"));
...
void World::render()
{
    ...
    mat4x4 transform = player->transform->get();
    S_obj->updateMat4("transform", transform);
